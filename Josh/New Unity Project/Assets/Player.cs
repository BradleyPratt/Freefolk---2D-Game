...
public class Player : MonoBehaviour {
    ...
    public int facing = 1; // 1 is right -1 is left
    ...
    void Update() {
        ...
        if (Input.GetButtonDown("Fire1"))
        {
            ...
            GameObject bullet_fired = Instantiate(weapon, (new Vector2(transform.position.x + 3f, transform.position.y)), Quaternion.identity) as GameObject;
        ...
        if (Input.GetButtonDown("Fire2"))
        {
            if (mana > 4)
            {
                ...
                GameObject bullet_fired = Instantiate(weapon, (new Vector2(transform.position.x + (5f * (float)facing), transform.position.y)), Quaternion.identity) as GameObject;
                bullet_fired.GetComponent<Rigidbody2D>().AddForce(new Vector2(50f * (float)facing, 1f), ForceMode2D.Impulse);
                mana -= 4;
            } else
            {
                //no mana
